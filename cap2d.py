...
class Dielectric(object):
    ...
    def draw(self, ax=None, **kw):
        ...
        x = np.array([l[0], l[0], r[0], r[0]])
        y = np.array([l[1], r[1], r[1], l[1]])
        
        lines = []
        line, = ax.plot(x[:,0], y[:,0], **kwargs)
        kwargs.update(color=line.get_color())
        lines.append(line)
...
class Rectangle(Dielectric):
    ...
    def __init__(self, bottom_left=(0, 0), sides=(1, 1), segments=(10, 10), **kw):
        ...
        super(Rectangle, self).__init__(bottom_left_x, bottom_left_y, width, height, **kw)        
